...
- (void)didReceiveuserdata:(OAServiceTicket*)ticket data:(NSData*)data {
    ...
    NSDictionary *user_data = [parser objectWithString:httpBody];
    ...
    NSString *profile_background_image_url = [user_data objectForKey:@"profile_background_image_url"];
    NSString *screen_name = [user_data objectForKey:@"screen_name"];
    NSString *friends_count = [user_data objectForKey:@"friends_count"];
    
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"Twitter User Data" message:[NSString stringWithFormat:@"Name = %@ : Followers-Count=%@ : Country=%@ : Profile-Image-Link=%@ : Screen-Name=%@ : Friends-Count=%@",name,followers,location,profile_background_image_url,screen_name,friends_count] delegate:nil cancelButtonTitle:nil otherButtonTitles:@"ok", nil];
    [alert show];
